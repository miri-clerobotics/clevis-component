...
우리 프로그램에서 사용하는 핵심 노드 컴포넌트의 동작 방식과 소스코드입니다.

## 1. 인터랙션 미리보기 (Live Preview)
아래 노드를 직접 **클릭**, **우클릭**, 혹은 **라벨을 드래그**하여 인터랙션을 테스트해보세요.
...
## 2. 노드 전체 소스코드 (React)
이 가이드라인을 구현한 전체 React 컴포넌트 소스코드입니다. 개발 시 참고하시기 바랍니다.
...
// ── Port data — left and right are independent ────────────────────────────────
...
function PortCircle({
  ...
  if (state === "default") {
    // When the paired label is in "clicked" state → gray circle (포트Item-2/8 spec)
  ...
  // hover or pressed — 18px ring indicator
...
function PortLabelLeft({
  ...
  if (state === "dragFollowing") {
    // Same height as default (34px = pt-2 + 10+20 lines + pb-2) to prevent row height change
    return (
      <div
        style={{
          width: 201,
  ...
  return (
    <div
      ...
      onMouseDown={onMouseDown}
    >
      {/* Inset border — absolute so it never affects layout or height */}
      {showBlueBorder && (
...
// ── Port row ──────────────────────────────────────────────────────────────────
// leftItem/rightItem can be undefined if one side has fewer items than the other.
function PortRow({
  leftItem, rightItem,
  ...
  const leftHoverRef  = useRef<ReturnType<typeof setTimeout>>();
  const rightHoverRef = useRef<ReturnType<typeof setTimeout>>();
  ...
  // On mousedown: notify App to handle single-selection (App clears all others),
  // then also start drag detection
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-[8px] px-[16px] w-full">
        ...
        {/* Play button — icon sized to match Figma: inner triangle at 61.5% of container */}
...
export default function App() {
  ...
  // ── Port state (left and right are independent) ───────────────────────────
  ...
  interface DragInfo {
    side: "left" | "right";
    sourceIndex: number;
    dropIndex: number; // insert-before index (0..n)
    x: number; y: number;
  }
  ...
  // Clicked state is momentary (like :active) — resets to default on mouseup
  const handlePortSelect = useCallback((side: "left" | "right", index: number) => {
    // Set clicked immediately on mousedown
    ...
    // Reset back to default on mouseup (wherever the mouse is released)
    const resetOnUp = () => {
  ...
  // Build drag start handler for a side + index
  const makeDragStart = useCallback((side: "left" | "right", index: number) => (e: React.MouseEvent) => {
    ...
    // Let rows update dropIndex via closure
    (window as any).__setPortDrop = (v: number) => { drop = v; setDragInfo(d => d ? { ...d, dropIndex: v } : null); };
    window.addEventListener("mousemove", move);
    window.addEventListener("mouseup", up);
  }, []);

  // Called when mouse enters a row area during drag — determines drop position
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            ...
            {/* Head — double-click to toggle accordion */}
            <NodeHead onDoubleClick={handleFoldToggle} />

            {/* Port section — hidden when collapsed or animating; visible when fully expanded */}
            <div
              style={{
            ...
            >
              {/* Port rows — left and right drag independently.
                  Inner div is position:relative so the absolute indicator doesn't shift rows. */}
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", paddingTop: 12, paddingBottom: 12, position: "relative" }}>
                ...
                {/* Absolute drop indicator — matches dragged label width, no layout shift.
                    Row height ≈ 34px (py-2 + 10px type + 20px name), gap = 4px → slot = 38px.
                    Left label: x=22 (after port circle), w=201.
                    Right label: x=235 (22+201+12 gap), w=201. */}
                {/* Drop indicator: 2px, centered inside the 4px gap.
                    Row h=34, gap=4, slot=38. Gap range: [row_end, row_end+4].
                    Center of gap = row_end + 2. Indicator top = center - 1 = 9 + dropIndex*38. */}
                {dragInfo && (
                  <div
                    style={{
                      position: "absolute",
            ...
            {/* Collapsed port count badges — hover shows tooltip */}
            {isCollapsed && (
              <>
                {/* Right badge */}
                <div
      ...
      >
        <span>마우스 올리기 → 호버</span>
        <span>클릭 → 선택 / 해제</span>
        <span>우클릭 → 컨텍스트 메뉴</span>
        <span>선택 후 툴바 첫 아이콘 → 접기 / 펼치기</span>
        <span>포트 라벨 호버 → 0.8s 후 툴팁</span>
        <span>포트 라벨 드래그 → 순서 변경</span>
        <span>포트 원 호버 / 누르기 → 엣지 생성 상태</span>
      </div>
    </div>
  );
}
